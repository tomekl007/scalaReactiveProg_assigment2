...
class EpidemySuite extends FunSuite {
  ...
  test("dead person stays dead"){
    ...
    println(s"--> choose person: $chosenOne , to be dead")
  ...
  test("life cycle"){
    var personDied = false;//to run set to true
    while(!personDied){
      val es = new EpidemySimulator
      ...
      println("find infected persons : " + infectedPerson)

      //before incubation time
    	while(es.agenda.head.time < incubationTime){
    	  println("time: "  + es.agenda.head.time)
  ...
  test("getNeightboursForRoom"){
    val es = new EpidemySimulator
    val row = 2
    val col = 4
    val expectedResult = List(2 -> 3, 2 -> 5, 1 -> 4, 3 -> 4)
    val result = es.getNeightboursForRoom(row, col)
    assert(expectedResult == result)
  }
  
  test("getNeightboursForRoom on edge"){
    val es = new EpidemySimulator
    val row = 8
    val col = 8
    val expectedResult = List(0 -> 7, 0 -> 1, 7 -> 0, 1 -> 0)
    val result = es.getNeightboursForRoom(row, col)
    println(result)
    assert(expectedResult == result)
  }
